...
df
qwer=df['총매출금액'].groupby(df['대분류명']).sum()
qwer
...
temp['a']=temp['new'].str.split('/').str[0].astype(float)//1
low=temp['총매출금액'].groupby(temp['a']).mean()

temp['b']=temp['new'].str.split('/').str[1].astype(float)//1
high=temp['총매출금액'].groupby(temp['b']).mean()
high.index
